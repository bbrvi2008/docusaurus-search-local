...
function getIndexHash(config) {
    ...
    if (config[flagField]) {
      for (const dir of config[dirField]) {
        if (!fs_1.default.existsSync(dir)) {
          ...
          files.push(
            ...klaw_sync_1.default(dir, { nodir: true, filter: markdownFilter })
          );
        }
      }
    }
  ...
  if (files.length > 0) {
    ...
    const pluginVersion = require(path_1.default.resolve(
      __dirname,
      "../../../../package.json"
    )).version;
    debug_1.debugInfo(
      "using @easyops-cn/docusaurus-search-local v%s",
      pluginVersion
    );
