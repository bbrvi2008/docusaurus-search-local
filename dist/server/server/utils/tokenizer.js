...
function tokenizer(input, metadata) {
  ...
  if (Array.isArray(input)) {
    return input.map(function (t) {
      return new lunr_1.default.Token(
        lunr_1.default.utils.asString(t).toLowerCase(),
        lunr_1.default.utils.clone(metadata)
      );
    });
  }
  ...
  while (text.length > 0) {
    ...
    if (/\w/.test(word[0])) {
      tokens.push(
        new lunr_1.default.Token(
          word,
          Object.assign(
            Object.assign({}, lunr_1.default.utils.clone(metadata)),
            { position: [start, word.length], index: tokens.length }
          )
        )
      );
      ...
      for (const zhWord of nodejieba_1.default.cut(word)) {
        tokens.push(
          new lunr_1.default.Token(
            ...
            Object.assign(
              Object.assign({}, lunr_1.default.utils.clone(metadata)),
              { position: [start, zhWord.length], index: tokens.length }
            )
          )
        );
        start += zhWord.length;
