...
const isStringOrArrayOfStrings = utils_validation_1.Joi.alternatives().try(
  utils_validation_1.Joi.string(),
  utils_validation_1.Joi.array().items(utils_validation_1.Joi.string())
);
const isArrayOfStringsOrRegExpsOrStringOrRegExp =
  utils_validation_1.Joi.alternatives().try(
    utils_validation_1.Joi.array().items(
      utils_validation_1.Joi.alternatives().try(
        utils_validation_1.Joi.string(),
        utils_validation_1.Joi.object().regex()
      )
    ),
    utils_validation_1.Joi.string(),
    utils_validation_1.Joi.object().regex()
  );
const translationsObject = utils_validation_1.Joi.object({
  ...
  search_results_for: utils_validation_1.Joi.string().default(
    'Search results for "{{ keyword }}"'
  ),
  search_the_documentation: utils_validation_1.Joi.string().default(
    "Search the documentation"
  ),
  count_documents_found_plural: utils_validation_1.Joi.string().default(
    (parent) => {
      var _a;
      return (_a = parent.count_documents_found) !== null && _a !== void 0
        ? _a
        : "{{ count }} documents found";
    }
  ),
  count_documents_found: utils_validation_1.Joi.string().default(
    "{{ count }} document found"
  ),
  no_documents_were_found: utils_validation_1.Joi.string().default(
...
const schema = utils_validation_1.Joi.object({
  ...
  highlightSearchTermsOnTargetPage:
    utils_validation_1.Joi.boolean().default(false),
  ...
  i18n: utils_validation_1.Joi.object()
    .pattern(utils_validation_1.Joi.string().min(2), translationsObject)
    .default({}),
});
